...
export const restartServiceTool: Tool = {
  definition: {
    ...
    description: '检查编译并安全重启当前服务（通过优雅退出让外部进程管理器重启）',
    parameters: {
      type: 'object',
      properties: {
        force: {
          type: 'boolean',
          description: '是否跳过编译检查强制重启',
          default: false
        },
        checkOnly: {
          type: 'boolean',
        ...
        delay: {
          type: 'integer',
          description: '重启延迟时间（毫秒，给响应返回留出时间）',
          default: 2000,
          minimum: 500,
          maximum: 10000
        }
  ...
  async execute(parameters: Record<string, any>): Promise<any> {
    const { force = false, checkOnly = false, timeout = 120, delay = 2000 } = parameters;
