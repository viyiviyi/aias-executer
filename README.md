...
AIAS Executor 现在包含强大的浏览器自动重连功能，确保浏览器会话在意外关闭后能够自动恢复。
...
2. **自动重连逻辑**
   - 浏览器关闭后，下次导航操作会自动重新打开浏览器
   - 最多重试2次，使用指数退避策略
   - 保留用户数据目录（cookies、localStorage等）
...
4. **事件监听器**
   - 监听Playwright的`disconnected`事件
   - 自动触发浏览器重新连接流程
   - 正确清理定时器和事件监听器

#### 使用示例

浏览器关闭后，`navigate_to_page`工具会自动重新打开浏览器：
...
当浏览器会话不存在时，现在会提供更详细的错误信息：

```
浏览器会话 my-session 不存在。可能的原因：
1. 浏览器已关闭或崩溃
2. 会话已过期（默认30分钟）
3. 从未创建过该会话
...
1. 使用 `navigate_to_page` 工具打开浏览器并导航到页面
2. 手动关闭浏览器窗口
3. 再次使用 `navigate_to_page` 工具导航到新页面
4. 验证浏览器是否自动重新打开并导航成功
